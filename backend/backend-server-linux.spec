...
# Backend server spec for Linux (Raspberry Pi)
import site
import os
...
if pycloudflared_path is None:
    raise SystemExit("ERROR: pycloudflared not found. Run: python3 -m pip install pycloudflared")
...
exe = EXE(
    ...
    a.datas,
    [],
    name='backend-server',   # No .exe on Linux
    debug=False,
