#!/usr/bin/env python3
"""
PM2200 Modbus Scanner & Reader
สแกนหา Register Addresses + แปลงค่าเป็น Human-readable
สำหรับ PM2200 Digital Meter ผ่าน RS485 (BR9600)
"""
...
class PM2200Scanner:
    ...
    # Register Map (PM2200 Modbus Protocol)
    # Verified by ON/OFF load comparison scan on real device
    # PM2200 uses 32-bit IEEE 754 float (2 registers per value, Big Endian)
    # NOTE: This PM2200 is connected single-phase, so per-phase L2/L3 power registers are empty.
    ...
    def connect(self) -> bool:
        """เชื่อมต่อไปยัง PM2200"""
        try:
            if self.client.connect():
                self.connected = True
                self.last_error = None
                logger.info(f"✅ Connected to PM2200 at {self.port}")
                return True
            else:
    ...
    def _decode_pf_quadrant(raw_pf: float):
        """
        PM2200 4-Quadrant Power Factor Encoding:
    ...
    def print_readable_data(self, data: Dict):
        ...
        """
        print("\n" + "=" * 70)
        print("📊 PM2200 - Power Monitoring Data")
        print("=" * 70)
...
if __name__ == "__main__":
    print("🔌 PM2200 Modbus Scanner & Reader")
    print("=" * 50)
...
REGISTER_MAP = {
    ...
    for name, info in PM2200Scanner.REGISTER_MAP.items()
}
...
class PM2200Client:
    """Wrapper around PM2200Scanner that returns flat dicts for the API."""

    def __init__(self, port: str = 'COM3', baudrate: int = 9600,
                 slave_id: int = 1, parity: str = 'E'):
        self._scanner = PM2200Scanner(port, baudrate, slave_id, parity)
